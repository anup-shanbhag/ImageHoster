...
public class UserRepository {
	...
	public void registerUser(User newUser) {
		...
		try {
			transaction.begin();
			// persist() method changes the state of the model object from transient state
			// to persistence state
			em.persist(newUser);
			transaction.commit();
		} catch (Exception e) {
	...
	// Creates an instance of EntityManager
	// Executes JPQL query to fetch the user from User class where username is equal
	// to received username and password is equal to received password
